...
pub fn make_fixture(path: &str) -> Result<(), umya_spreadsheet::XlsxError> {
    ...
    sheet.get_cell_mut((3, 1)).set_value("tema");
    sheet.get_cell_mut((4, 1)).set_value("type");
    sheet.get_cell_mut((5, 1)).set_value("order");
    sheet.get_cell_mut((6, 1)).set_value("locale");
    sheet.get_cell_mut((7, 1)).set_value("abstract");
    sheet.get_cell_mut((1, 2)).set_value("f1");
    sheet.get_cell_mut((2, 2)).set_value("Fixture One");
    sheet.get_cell_mut((3, 2)).set_value("Miljø");
    sheet.get_cell_mut((4, 2)).set_value("Poster");
    sheet.get_cell_mut((5, 2)).set_value(1);
    sheet.get_cell_mut((6, 2)).set_value("en");
    sheet.get_cell_mut((7, 2)).set_value("Text one");
